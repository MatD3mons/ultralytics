...
class BaseTrainer:
    ...
    Attributes:
        ...
        valset (torch.utils.data.Dataset): Testing dataset.
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.check_resume()
        self.validator = None
        #self.testor= None #TODO add testor
        self.model = None
        self.metrics = None
        self.plots = {}
        ...
        ##################################################################################################
        #TODO ADD valset and testset ( the both)
        self.trainset, self.valset, self.testset, self.support = self.get_dataset(self.data) # TODO add support output here
        ##################################################################################################
        self.ema = None
    ...
    def _setup_train(self, world_size):
        ...
        if self.args.task == 'oneshot':
            self.train_loader = self.get_dataloader(self.trainset, self.support, batch_size=batch_size, rank=RANK, mode='train')
        else:
        ...
        if RANK in (-1, 0):
            ...
            #TODO here add supp
            if self.args.task == 'oneshot':
                self.val_loader = self.get_dataloader(self.valset, self.support, batch_size=batch_size * 2, rank=-1, mode='val')
                #self.test_loader = self.get_dataloader(self.testset, self.support, batch_size=batch_size * 2, rank=-1, mode='val') #TODO add testor
            else:
                self.val_loader = self.get_dataloader(self.valset, batch_size=batch_size * 2, rank=-1, mode='val')
        ...
        self.scheduler = optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=self.lf) #BUG TODO
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if RANK in (-1, 0):
                ...
                #if self.args.val or final_epoch:
                #    self.metrics, self.fitness = self.test()
    ...
    def get_dataset(data):
        ...
        """
        ##################################################################################################
        return data['train'], data.get('val'), data.get('test'), data['support'] # TODO add support here
        ##################################################################################################
