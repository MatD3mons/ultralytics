...
class BasePredictor:
    ...
    def __call__(self, source=None, support=None, model=None, stream=False): 
        """Performs inference on an image or stream."""
        self.stream = stream
        if stream: 
            return self.stream_inference(source, support, model) #TODO add support.
        else:
            return list(self.stream_inference(source, support, model))  # merge list of Result into one #TODO add support.

    def predict_cli(self, source=None, support=None, model=None):
        """Method used for CLI prediction. It uses always generator as outputs as not required by CLI mode."""
        gen = self.stream_inference(source, support, model)
        for _ in gen:  # running CLI inference without accumulating any outputs (do not modify)
            pass

    def setup_source(self, source, support=None):
        """Sets up source and inference mode."""
        self.imgsz = check_imgsz(self.args.imgsz, stride=self.model.stride, min_dim=2)  # check image size
        self.transforms = getattr(self.model.model, 'transforms', classify_transforms(
            self.imgsz[0])) if self.args.task == 'classify' else None
        self.dataset = load_inference_source(source=source, support=support, imgsz=self.imgsz, vid_stride=self.args.vid_stride)
    ...
    def stream_inference(self, source=None, support=None, model=None): #TODO add support.
        ...
        ##############################################################################
        # TODO here add support
        self.setup_source(source if source is not None else self.args.source, support=support)
        ##############################################################################
        ...
        for batch in self.dataset:
            ...
            ##################################################################
            if(len(batch) == 5):
                path, im0s, sup0s, vid_cap, s = batch #TODO add supp after imgs
            else:
            ...
            with profilers[1]:
                ##############################################################################
                if len(batch) == 5:
                    preds = self.model(im, sup, augment=self.args.augment, visualize=visualize)
                else:
