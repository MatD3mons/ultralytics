...
class LoadPilAndNumpy:
    ...
    def __init__(self, im0, sup0, imgsz=640):
        """Initialize PIL and Numpy Dataloader."""
        if not isinstance(im0, list):
            im0 = [im0]
        if not isinstance(sup0, list):
            sup0 = [sup0]
        self.paths = [getattr(im, 'filename', f'image{i}.jpg') for i, im in enumerate(im0)]
        self.im0 = [self._single_check(im) for im in im0]
        self.sup0 = [self._single_check(sup) for sup in sup0]
        self.imgsz = imgsz
        self.mode = 'image'
        # Generate fake paths
    ...
    def __next__(self):
        ...
        if self.count == 1:  # loop only once as it's batch inference
            raise StopIteration
        self.count += 1
        return self.paths, self.im0, self.sup0, None, ''
...
class LoadTensor:
    ...
    def __init__(self, im0, sup0) -> None:
        self.im0 = self._single_check(im0)
        self.sup0 = self._single_check(sup0)
