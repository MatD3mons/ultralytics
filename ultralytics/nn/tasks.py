...
####################################### TODO add this new modele
class OneShotDetectionModel(BaseModel):
    """YOLOv8 OneShot object detection model."""
    ...
    #TODO add this function because we overwrite
    def forward(self, x, y=None, *args, **kwargs):
        ...
        Args:
            x (torch.Tensor | dict): The input image tensor or a dict including image tensor and gt labels.
            y (torch.Tensor | dict): The support image tensor or a dict including image tensor and gt labels.

        Returns:
            (torch.Tensor): The output of the network.
        """
        if isinstance(x, dict):  # for cases of training and validating while training.
            return self.loss(x, *args, **kwargs) #TODO le supp est dans le batch...
        return self.predict(x, y, *args, **kwargs) 

    def loss(self, batch, preds=None):#TODO add overwrite fonction
        """
        Compute loss
        ...
        #TODO supp is adding
        ###########################################################################
        preds = self.forward(batch['img'], batch['sup']) if preds is None else preds
        ############################################################################
        return self.criterion(preds, batch)

    def predict(self, x, y, profile=False, visualize=False, augment=False):
        """
        Perform a forward pass through the network.

        Args:
            x (torch.Tensor): The input tensor to the model.
            y (torch.Tensor): The support tensor to the model.
            profile (bool):  Print the computation time of each layer if True, defaults to False.
            visualize (bool): Save the feature maps of the model if True, defaults to False.
            augment (bool): Augment image during prediction, defaults to False.
    ...
    def _predict_once(self, x, y, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            xz.append(x if m.i in self.save else None)  # save output
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    return mySequential(*layers), sorted(save)
