...
class AutoBackend(nn.Module):
    ...
    def forward(self, im, sup=None, augment=False, visualize=False):
        """
        Runs inference on the YOLOv8 MultiBackend model.

        Args:
            im (torch.Tensor): The image tensor to perform inference on.
            sup (torch.Tensor): The support image tensor to guide the prediction.
        ...
        if self.pt or self.nn_module:  # PyTorch
            if sup is not None: #TODO add here condition for supp
                y = self.model(im, sup, augment=augment, visualize=visualize) if augment or visualize else self.model(im,sup)
            else:
    ...
    def warmup(self, imgsz=(1, 3, 640, 640), supsz=None): #TODO here add support entry
        """
        Warm up the model by running one forward pass with a dummy input.

        Args:
            imgsz (tuple): The shape of the dummy input tensor in the format (batch_size, channels, height, width)
            imgsz (tuple): The shape of the dummy support tensor in the format (batch_size, channels, height, width)
        ...
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            ...
            #TODO add here the conditon if use oneshot
            if supsz is not None:
                sup = torch.empty(*supsz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # support
            for _ in range(2 if self.jit else 1):  #
                #TODO add here the conditon if use oneshot
                if supsz is not None:
                    self.forward(im, sup)  # warmup
                else:
